...
def train_epoch(epoch, client, config, time_start, input_A, input_B, target_real, target_fake):
...
def train_federated(config, clients, server):
    ...
    # Inputs & targets momory allocation 
    # Cuda setup
    if config.no_cuda:
    ...
    input_A = FloatTensor(config.batch_size, config.input_nc, config.img_size, config.img_size)
    input_B = FloatTensor(config.batch_size, config.input_nc, config.img_size, config.img_size)
    target_real = Variable(FloatTensor(config.batch_size).fill_(1.0), requires_grad=False)
    target_fake = Variable(FloatTensor(config.batch_size).fill_(0.0), requires_grad=False)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for client in clients:
            ...
            loss_dict = train_epoch(epoch=epoch, client=client, config=config, time_start=time_start, input_A=input_A, input_B=input_B, target_real=target_real, target_fake=target_fake)
            time_end = time.time()
            config.epoch_eta = time_end - time_start
        ...
        if epoch % config.com_epochs == 0:
            ...
            server.generator_A2B.load_state_dict(para_global_G_A2B)
            server.generator_B2A.load_state_dict(para_global_G_B2A)
            server.discriminator_A.load_state_dict(para_global_D_A)
            server.discriminator_B.load_state_dict(para_global_D_B)
